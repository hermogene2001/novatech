...
try {
    ...
    // Fetch agent bank details
    $stmt = $pdo->prepare("SELECT * FROM user_banks WHERE user_id = ?");
    $stmt->execute([$agent_id]);
    $bank = $stmt->fetch(PDO::FETCH_ASSOC);
} catch(PDOException $e) {
    $error = "Error fetching agent data: " . $e->getMessage();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_profile'])) {
    ...
    try {
        ...
        $stmt->execute([$first_name, $last_name, $email, $agent_id]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_bank'])) {
    ...
    try {
        if ($bank) {
            ...
            $stmt->execute([$bank_name, $account_number, $account_holder, $agent_id]);
        } else {
            // Insert new bank details
            $stmt = $pdo->prepare("INSERT INTO user_banks (user_id, bank_name, account_number, account_holder) VALUES (?, ?, ?, ?)");
            $stmt->execute([$agent_id, $bank_name, $account_number, $account_holder]);
        }
...
<head>
    ...
    <title>Agent Profile - Novatech Investment Platform</title>
        ...
        <div class="row mt-4">
                    ...
                    <div class="card-body">
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="first_name" name="first_name" value="<?php echo htmlspecialchars($agent['first_name']); ?>" required>
                            </div>
                            
                            <div class="mb-3">
                                <label for="last_name" class="form-label">Last Name</label>
                                <input type="text" class="form-control" id="last_name" name="last_name" value="<?php echo htmlspecialchars($agent['last_name']); ?>" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="phone_number" name="phone_number" value="<?php echo htmlspecialchars($agent['phone_number']); ?>" disabled>
                            </div>
            ...
            <div class="col-md-6">
                <div class="card mb-4">
                    <div class="card-header">
                        <h4>Bank Details</h4>
                        <p class="mb-0"><small class="text-muted">These details will be shown to clients when they make deposits</small></p>
                    </div>
                    <div class="card-body">
                        <form method="POST">
